...
int main()
{
    ...
    do
    {
        ...
        case 1:
        {
            acc[Enhanced_Account_Tracking_System::getTotalAccounts()].add_new_account(acc); // logic is not correct but it is not working
            break;
        }
        ...
        case 8:
            for (int i = 0; i < n; i++)
            {
                cout << "Account " << acc[i].getAccountNumber() << " has "
                     << acc[i].getTransactionCount() << " transactions." << endl;
            }
            break;
